...
This pass (2026-09-17 ~1:53 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1131** at confirmed round **104** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1132**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=110. LocalNet last-round after listen: 111. History sample #18 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
