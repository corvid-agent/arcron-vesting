...
This pass (2026-09-04 ~11:07 AM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1014** at confirmed round **11** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1015**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=17. LocalNet last-round after listen: 18. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
