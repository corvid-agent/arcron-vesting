...
This pass (2026-09-11 ~4:48 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1092** at confirmed round **73** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1093**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=79. LocalNet last-round after listen: 80. History sample #17 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
