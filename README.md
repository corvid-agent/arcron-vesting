...
This pass (2026-09-09 ~5:49 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1103** at confirmed round **84** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1104**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=90. LocalNet last-round after listen: 91. History sample #13 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
