...
This pass (2026-09-10 ~5:45 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1029** at confirmed round **24** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1030**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=30. LocalNet last-round after listen: 31. History sample #15 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
