...
This pass (2026-09-11 ~11:58 AM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1068** at confirmed round **55** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1069**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=61. LocalNet last-round after listen: 62. History sample #16 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
