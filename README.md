...
This pass (2026-09-08 ~9:54 AM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1162** at confirmed round **128** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1163**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=134. LocalNet last-round after listen: 135. History sample #11 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
