...
This pass (2026-09-07 ~1:48 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1123** at confirmed round **97** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1124**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=103. LocalNet last-round after listen: 104. History sample #10 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
