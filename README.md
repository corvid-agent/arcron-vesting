...
This pass (2026-09-04 ~3:55 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1053** at confirmed round **42** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1054**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=48. LocalNet last-round after listen: 49. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
