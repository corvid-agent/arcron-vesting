...
This pass (2026-09-10 ~12:02 PM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1016** at confirmed round **14** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1017**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=20. LocalNet last-round after listen: 21. History sample #14 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
