...
This pass (2026-09-07 ~8:52 AM MT): `python scripts/localnet_recreate.py` created Vesting **appId 1084** at confirmed round **66** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1085**, inner-called `accrue` (1 inner), and the beneficiary pulled. Global after listen: locked=credited=claimed=1_000_000 µALGO, last_accrue_round=72. LocalNet last-round after listen: 73. History sample #9 appended. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
